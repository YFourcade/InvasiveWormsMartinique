...
ggsave(filename = "synthetic_maps.png", width = 10, height = 4.5)
...
p_d_env <- overlap_env %>% 
  left_join(sp.char, by = c("Introduced" = "Species")) %>% rename(Habitat_intro = Habitat) %>% 
  left_join(sp.char, by = c("Native" = "Species")) %>% rename(Habitat_native = Habitat) %>% 
  filter(Habitat_intro == Habitat_native) %>% 
  ...
  facet_grid(~ Habitat_intro, scales = "free") +
  theme(strip.text = element_text(face = 'bold'),
